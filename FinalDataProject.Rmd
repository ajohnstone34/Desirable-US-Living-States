...
```{r}
introduce(df_clean)
custom_config <- configure_report(
  plot_str_args = list(
    type = "diagonal",
    width = 1000,
    margin = list(left = 400, right = 250) 
  )
)
create_report(
  data = df_clean,
...
```{r}
results <- describe(df_clean)

write.csv(results, file = "summary_stats.csv", row.names = TRUE)
```
...
create_report(
  df_clean,
  config = custom_config,
  output_file = "custom_data_report.html",
  report_title = "Custom Data Profiling Report"
)
```

```{r}
## implementing PCA to check for possible dimensionality reduction
X <- subset(df_clean, select = -overall_state_rank)
y <- df_clean$overall_state_rank
...
##fviz_eig(pca_result)
fa.parallel(df_clean, fa = "pc", n.iter = 50)
##plot(pca_result, type = "l", main = "Scree Plot")
```

```{r}
df_clean$overall_state_rank <- as.factor(df_clean$overall_state_rank)

levels(df_clean$overall_state_rank) <- c("no", "yes")

# Extract the first 5 comp
reduced_data <- as.data.frame(pca_result$x[, 1:5])
colnames(reduced_data) <- colnames(pca_result$rotation)[1:5]

# Adding dep variable back
reduced_data$overall_state_rank <- df_clean$overall_state_rank

```

```{r}
##LDA testing using kfold
tune_grid <- expand.grid(decay = c(0, 0.01, 0.1, 0.5))

index <- createDataPartition(df_clean$overall_state_rank, p = 0.8, list = FALSE)
train_set <- df_clean[index, ]
test_set  <- df_clean[-index, ]

lda_df_clean <- df_clean

lda_df_clean$overall_state_rank <- as.factor(lda_df_clean$overall_state_rank)

levels(lda_df_clean$overall_state_rank) <- c("no", "yes")
...
## Fit LDA model on unscaled predictors
lda_model_cv <- train(
  overall_state_rank ~ .,            
  data = train_set,            
  method = "lda",         
...
predictions <- predict(lda_model_cv, newdata = test_set)

conf_matrix <-confusionMatrix(predictions, test_set$overall_state_rank)
print(conf_matrix)

fourfoldplot(as.table(conf_matrix), color = c("red", "green"), main = "Confusion Matrix")

```

```{r}
## k fold cross val and glm model 
...
##glm_model_cv <- glm(overall_state_rank ~ ., family = binomial, data = df_clean)

print(glm_model_cv)

# 4. Use your code to predict on the UNSEEN test_set
predictions <- predict(glm_model_cv, newdata = test_set)
...
# 2. Configure 10-fold cross-validation
train_control <- trainControl(
  method = "cv",          
  number = 10, 
  classProbs = TRUE,      
...
# 3. Train on train_set using XGBoost
# xgb_model_cv <- train(
#   overall_state_rank ~ .,            
  # data = train_set,
#   method = "xgbTree",            # Use XGBoost tree method
#   trControl = train_control,
#   metric = "ROC",                # Primary metric for binary classification
#   tuneLength = 3,                # Tests 3 random hyperparameter combinations
#   na.action = na.pass,           # Allows XGBoost to handle missing values natively
#   verbosity = 0                  # Silences technical logs
# )
# 4. Predict on the UNSEEN test_set
# predictions <- predict(xgb_model_cv, newdata = test_set)

# 5. Final evaluation
# conf_matrix <- confusionMatrix(predictions, test_set$overall_state_rank)
# print(conf_matrix)

# 6. Check Variable Importance (highly recommended for XGBoost)
# print(varImp(xgb_model_cv))
...
# 3. Train on train_set using Random Forest
# Note: 'rf' uses the randomForest package; 'ranger' is a faster alternative
rf_model_cv <- train(
  overall_state_rank ~ .,            
  data = train_set,
  ...
  trControl = train_control,
  metric = "ROC",                
  tuneLength = 3,                # Tries different values for 'mtry'
  preProcess = "medianImpute",   # Recommended: RF requires complete data
  ntree = 500                    # Optional: Specify number of trees
)

# 4. Predict on the UNSEEN test_set
predictions <- predict(rf_model_cv, newdata = test_set)
...
# 3. Train on train_set using Multinomial Logistic Regression
multinom_model_cv <- train(
  overall_state_rank ~ .,            
  data = train_set,
  method = "multinom",           # Use Multinomial approach
  trControl = train_control,
  tuneGrid = tune_grid,
  metric = "ROC",                
  trace = FALSE,                 # Suppresses the detailed iteration output
  preProcess = c("center", "scale"), # Recommended for regression models
  tuneLength = 3                 # Tries different weight decay values
)
...
# 4. Predict on the UNSEEN test_set
predictions <- predict(multinom_model_cv, newdata = test_set)
...
```{r}
# Extract coefficients from the underlying nnet model
coeffs <- summary(multinom_model_cv$finalModel)$coefficients

# Check for unusually large absolute values (e.g., > 10)
print(max(abs(coeffs)))

# View the performance across all 50 repeats
print(multinom_model_cv$results)
```

```{r}
# 1. Get predictions for both sets
train_preds <- predict(multinom_model_cv, newdata = train_set)
test_preds <- predict(multinom_model_cv, newdata = test_set)

# 2. Calculate accuracies
train_acc <- mean(train_preds == train_set$overall_state_rank)
test_acc <- mean(test_preds == test_set$overall_state_rank)

# 3. Print comparison
print(paste("Training Accuracy:", round(train_acc, 4)))
print(paste("Testing Accuracy:", round(test_acc, 4)))
print(paste("Gap:", round(train_acc - test_acc, 4)))
```

```{r}
AIC(multinom_model_cv$finalModel)
```
...
# 3. Train on train_set using Neural Network (nnet)
nn_model_cv <- train(
  overall_state_rank ~ .,            
  data = train_set,
  ...
  trControl = train_control,
  metric = "ROC",                
  preProcess = c("center", "scale"), # Mandatory: NNs require normalized data
  tuneLength = 5,                # Tunes 'size' (nodes) and 'decay' (regularization)
  trace = FALSE,                 # Suppress iteration logs
  MaxNWts = 5000,                # Increase limit for weights if you have many predictors
  maxit = 200                    # Increase iterations to ensure convergence
)
...
# 4. Predict on the UNSEEN test_set
predictions <- predict(nn_model_cv, newdata = test_set)
...
```{r}
# 1. Get predictions for both sets
train_preds <- predict(nn_model_cv, newdata = train_set)
test_preds <- predict(nn_model_cv, newdata = test_set)
...
# View results of your caret neural network (e.g., method = 'nnet')
print(nn_model_cv$results)

##AIC(nn_model_cv$finalModel)

```

```{r}
##multinom BUT FINE TUNED

#index <- createDataPartition(df_clean$overall_state_rank, p = 0.8, list = FALSE)
#train_set <- df_clean[index, ]
#test_set  <- df_clean[-index, ]

# 1. Define a Tuning Grid for Regularization (Weight Decay)
# Decay values usually range from 0 (no penalty) to 0.5. 
# Increasing this value prevents overfitting.
tune_grid <- expand.grid(decay = c(0, 0.001, 0.01, 0.1, 0.2, 0.5))

# 2. Configure Cross-Validation
# Added 'selectionFunction = "oneSE"' - this picks the simplest model 
# within one standard error of the best, a classic technique to avoid overfitting.
train_control <- trainControl(
  method = "repeatedcv",          
  number = 10, 
  repeats = 5,
  ...
  allowParallel = TRUE,
  selectionFunction = "oneSE"           # Favors simpler models to prevent overfitting
)

# 3. Train the model with Tuning and Pre-processing
multinom_model_cv <- train(
  overall_state_rank ~ .,            
  data = train_set,
  method = "multinom",           
  trControl = train_control,
  tuneGrid = tune_grid,                # Applied the regularization grid
  metric = "Accuracy",                 # 'ROC' is usually for binary; use 'Accuracy' or 'Kappa' for multinom
  trace = FALSE,                 
  preProcess = c("center", "scale", "nzv"), # 'nzv' removes variables with near-zero variance
  maxit = 1000                         # Increase iterations to ensure convergence
)

# View the best tuning parameters
print(multinom_model_cv$bestTune)
...
```{r}
## neuralnet BUT FINE TUNED

# 1. Define a specific grid for size and decay
# 'size': hidden nodes (keep small to avoid overfitting)
# 'decay': weight decay (higher values provide stronger regularization)
nn_grid <- expand.grid(
  size = c(1, 3, 5), 
  decay = c(0.1, 0.5, 1.0)
)
...
train_control <- trainControl(
  ...
  repeats = 5,
  classProbs = TRUE,      
  summaryFunction = multiClassSummary, # Use multiClassSummary for multi-rank data
  allowParallel = TRUE,
  selectionFunction = "oneSE"           # SELECTS THE SIMPLEST MODEL within 1 SE of the best
)
...
nn_model_cv <- train(
  ...
  method = "nnet",               
  trControl = train_control,
  tuneGrid = nn_grid,            # Use the explicit grid instead of tuneLength
  metric = "Accuracy",           # Use Accuracy/Kappa for multi-class ranking
  preProcess = c("center", "scale", "nzv"), # 'nzv' removes low-information predictors
  trace = FALSE,                 
  MaxNWts = 5000,                
  maxit = 500                    # Increased iterations for better convergence
)

# View the best parameters chosen
print(nn_model_cv$bestTune)
